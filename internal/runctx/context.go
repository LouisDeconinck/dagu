...
// WithSecretEnvVars sets execution-scoped secret environment variables. It is
// the secret counterpart to WithEnvVars: the values sit at the same precedence
// as execution-scoped env vars, so the run's own secrets still win by name.
func WithSecretEnvVars(secrets ...string) ContextOption {
	return func(o *contextOptions) {
		o.selectedSecretEnvs = append(o.selectedSecretEnvs, secrets...)
	}
}
...
func NewContext(
	...
	selectedSecretEnvs := stringutil.KeyValuesToMap(options.selectedSecretEnvs)
	...
	// Execution-scoped secrets sit with the execution-scoped env vars they
	// arrive alongside, so both behave the same against the run's own values,
	// while carrying the secret source that drives masking.
	if len(selectedSecretEnvs) > 0 {
		scope = scope.WithEntries(selectedSecretEnvs, cmnvalue.EnvSourceSecret)
	}
